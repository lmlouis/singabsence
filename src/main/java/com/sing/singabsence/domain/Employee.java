...
public class Employee implements Serializable {
    ...
    @ManyToMany(fetch = FetchType.LAZY, mappedBy = "senttos")
    @Cache(usage = CacheConcurrencyStrategy.READ_WRITE)
    @JsonIgnoreProperties(
        value = { "owner", "workflow", "reason", "period", "senttos", "ownworkflows", "requestedbies" },
        allowSetters = true
    ...
    public Employee request(Leave leave) {
    ...
    public void setRequests(Set<Leave> leaves) {
        if (this.requests != null) {
            this.requests.forEach(i -> i.removeSentto(this));
        }
        if (leaves != null) {
            leaves.forEach(i -> i.addSentto(this));
        }
    ...
    public Employee addRequests(Leave leave) {
        this.requests.add(leave);
        leave.getSenttos().add(this);
        return this;
    }

    public Employee removeRequests(Leave leave) {
        this.requests.remove(leave);
        leave.getSenttos().remove(this);
        return this;
    }
