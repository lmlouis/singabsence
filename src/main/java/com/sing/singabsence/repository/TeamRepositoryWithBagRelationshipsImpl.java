...
public class TeamRepositoryWithBagRelationshipsImpl implements TeamRepositoryWithBagRelationships {
    ...
    public Optional<Team> fetchBagRelationships(Optional<Team> team) {
        return team.map(this::fetchMembers);
    }
    ...
    public List<Team> fetchBagRelationships(List<Team> teams) {
        return Optional.of(teams).map(this::fetchMembers).orElse(Collections.emptyList());
    }

    Team fetchMembers(Team result) {
        return entityManager
            .createQuery("select team from Team team left join fetch team.members where team.id = :id", Team.class)
    ...
    List<Team> fetchMembers(List<Team> teams) {
        ...
        List<Team> result = entityManager
            .createQuery("select team from Team team left join fetch team.members where team in :teams", Team.class)
