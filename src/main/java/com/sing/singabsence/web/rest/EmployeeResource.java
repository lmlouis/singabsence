...
/**
 * REST controller for managing {@link com.sing.singabsence.domain.Employee}.
 */
@RestController
...
public class EmployeeResource {
    ...
    public List<EmployeeDTO> getAllEmployees(
        ...
        if ("ownenterprise-is-null".equals(filter)) {
            log.debug("REST request to get all Employees where ownenterprise is null");
            return employeeService.findAllWhereOwnenterpriseIsNull();
        }

        if ("ownteam-is-null".equals(filter)) {
            log.debug("REST request to get all Employees where ownteam is null");
            return employeeService.findAllWhereOwnteamIsNull();
        }

        if ("ownmessage-is-null".equals(filter)) {
            log.debug("REST request to get all Employees where ownmessage is null");
            return employeeService.findAllWhereOwnmessageIsNull();
        }

        if ("request-is-null".equals(filter)) {
            log.debug("REST request to get all Employees where request is null");
            return employeeService.findAllWhereRequestIsNull();
        }
