...
describe('Message Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call Employee query and add missing value', () => {
      ...
      const tos: IEmployee[] = [{ id: 12118 }];
      message.tos = tos;

      const employeeCollection: IEmployee[] = [{ id: 11082 }];
      jest.spyOn(employeeService, 'query').mockReturnValue(of(new HttpResponse({ body: employeeCollection })));
      const additionalEmployees = [...tos];
    ...
    it('Should call from query and add missing value', () => {
      const message: IMessage = { id: 456 };
      const from: IEmployee = { id: 10105 };
      message.from = from;

      const fromCollection: IEmployee[] = [{ id: 14167 }];
      jest.spyOn(employeeService, 'query').mockReturnValue(of(new HttpResponse({ body: fromCollection })));
      const expectedCollection: IEmployee[] = [from, ...fromCollection];
      jest.spyOn(employeeService, 'addEmployeeToCollectionIfMissing').mockReturnValue(expectedCollection);
    ...
    it('Should update editForm', () => {
      const message: IMessage = { id: 456 };
      const from: IEmployee = { id: 3882 };
      message.from = from;
      const to: IEmployee = { id: 24646 };
      message.tos = [to];
      const attachements: IAttachment = { id: 23441 };
      message.attachements = [attachements];
      ...
      expect(comp.employeesSharedCollection).toContain(to);
      expect(comp.attachmentsSharedCollection).toContain(attachements);
