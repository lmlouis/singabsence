...
type RestOf<T extends IEvent | NewEvent> = Omit<T, 'start' | 'end'> & {
  start?: string | null;
  end?: string | null;
};
...
export class EventService {
  ...
  protected convertDateFromClient<T extends IEvent | NewEvent | PartialUpdateEvent>(event: T): RestOf<T> {
    return {
      ...event,
      start: event.start?.toJSON() ?? null,
      end: event.end?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEvent: RestEvent): IEvent {
    return {
      ...restEvent,
      start: restEvent.start ? dayjs(restEvent.start) : undefined,
      end: restEvent.end ? dayjs(restEvent.end) : undefined,
    };
  }
