...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'birthday'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...employee,
      birthday: employee.birthday?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...restEmployee,
      birthday: restEmployee.birthday ? dayjs(restEmployee.birthday) : undefined,
    };
  }
